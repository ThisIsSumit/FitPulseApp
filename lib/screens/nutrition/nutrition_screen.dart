...
class _NutritionScreenState extends State<NutritionScreen>
  ...
  void initState() {
    ...
    SB.nutritionLogsStream(uid).listen((data) {
      if (mounted)
        setState(() => _logs = data.map(NutritionLog.fromMap).toList());
    });
  }
    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      ...
      if (logs.isEmpty)
        ...
        ...logs
            .asMap()
            .entries
            .map((e) => _MealTile(log: e.value, index: e.key)),
    ]);
  }
}
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          Text(
              '${log.mealType} · P:${log.protein.toInt()}g C:${log.carbs.toInt()}g F:${log.fat.toInt()}g',
...
class _WeeklyTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final today =
        DateTime(now.year, now.month, now.day); // strip time → midnight
    final Map<int, int> dayCalories = {for (var i = 0; i < 7; i++) i: 0};

    for (final l in logs) {
      final logDate =
          DateTime(l.date.year, l.date.month, l.date.day); // strip time
      final diff =
          today.difference(logDate).inDays; // now a clean integer day count
      if (diff >= 0 && diff < 7) {
        dayCalories[6 - diff] = (dayCalories[6 - diff] ?? 0) + l.calories;
      }
    }
    ...
    // Build day labels relative to "today", matching how bars are filled (6-diff)
    final dayLabels = List.generate(7, (i) {
      final d = today.subtract(Duration(days: 6 - i));
      return DateFormat('E')
          .format(d)
          .substring(0, 3); // Mon, Tue, etc. — always correct
    });
    final todayIdx = 6; // today is always the last bar now
          ...
          child: BarChart(BarChartData(
            ...
            gridData: FlGridData(
                ...
                getDrawingHorizontalLine: (_) =>
                    FlLine(color: AppColors.bgSurface, strokeWidth: 1)),
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                 ...
                 getTitlesWidget: (v, _) => Text(dayLabels[v.toInt() % 7],
                    style: TextStyle(
                        color: v.toInt() == todayIdx
                            ? AppColors.primary
                            : AppColors.textMuted,
                        fontSize: 11,
                        fontWeight: v.toInt() == todayIdx
                            ? FontWeight.w700
                            : FontWeight.w400),
                  ),
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
        ...
        children: grouped.entries
            .map((e) => _MealGroup(type: e.key, logs: e.value))
            .toList());
  }
}
    ...
    return Padding(
      ...
      child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            GradientButton(
                label: 'Save Meal', onTap: _save, isLoading: _loading),
          ]),
    );
  }
}
